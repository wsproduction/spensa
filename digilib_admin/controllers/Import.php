...
class Import extends Controller {
    ...
    public function student() {
        $inputFileName = Web::path() . 'asset/upload/file/kls7.xls';
        if (file_exists($inputFileName)) {
            ...
            try {
                ...
                for ($i = 1; $i <= $count_student; $i++) {
                    ...
                    $jk = $sheet->getCell('C' . $numrow)->getValue();
                    $place = $sheet->getCell('E' . $numrow)->getValue();
                    $tgl = $sheet->getCell('F' . $numrow)->getValue();
                    $address = $sheet->getCell('L' . $numrow)->getValue();
                    $phone = $sheet->getCell('M' . $numrow)->getValue();
                    ...
                    $data['name'] = ucwords(strtolower($nama));
                    $data['gender'] = $jk_id;
                    $data['birthplace'] = $place;
                    $data['birthdate'] = date('Y-m-d', strtotime($tgl));
