...
class Employee extends Controller {
    ...
    public function index() {
        Web::setTitle('Daftar Hadir Tata Usaha');
        $this->view->link_r = $this->content->setLink('employee/read');
        $this->view->link_c = $this->content->setLink('employee/add');
        $this->view->link_d = $this->content->setLink('employee/delete');
        $this->view->option_name = $this->content->optionName(3);
    ...
    public function read() {
        if ($this->method->isAjax()) {
            ...
            if ($action == 999) {
                ...
                $userSpeday = $this->content->parsingSpeday($nameid, $begin->format('m/d/Y'), $end->format('m/d/Y'));
                $holidays = $this->content->parsingHolidays($begin->format('m/d/Y'), $end->format('m/d/Y'));

                $listView = $this->content->parsingListView($listData, $dateList, $checkInOut, $userSpeday, $holidays);
